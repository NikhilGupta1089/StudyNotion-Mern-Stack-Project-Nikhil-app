            ...
            if (key === "tag" || key === "instructions") {
              if (typeof updates[key] === "string") {
                try {
                  ...
                  course[key] = []; // fallback to empty array
                }
              } else {
                course[key] = updates[key];
              }
    ...
    const coursesWithDuration = instructorCourses.map((course) => {
      ...
      course.courseContent.forEach((section) => {
        section.subSection.forEach((subSec) => {
          // Assuming duration is in "mm:ss" or just minutes
          if (typeof subSec.timeDuration === "string") {
            const parts = subSec.timeDuration.split(":")
            if (parts.length === 2) {
              const [min, sec] = parts.map(Number)
              totalDurationInSeconds += min * 60 + sec
            } else {
              totalDurationInSeconds += Number(subSec.timeDuration) * 60
            }
          } else if (typeof subSec.timeDuration === "number") {
            totalDurationInSeconds += subSec.timeDuration
          }
        })
      })
      ...
      return {
        ...course._doc,
        totalDurationInMinutes: Math.round(totalDurationInSeconds / 60),
      }
    })
